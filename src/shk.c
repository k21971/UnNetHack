...
static void getcad(struct monst *, const char *, coordxy, coordxy, boolean,
                   boolean, boolean);
...
static void clear_no_charge_pets(struct monst *);
...
static int QSORTCALLBACK sortbill_cmp(const genericptr, const genericptr) NONNULLPTRS;
static long cheapest_item(int, Bill *) NONNULLPTRS;
static int make_itemized_bill(struct monst *shkp, Bill **ibill) NONNULLPTRS;
static int menu_pick_pay_items(int, Bill *) NONNULLPTRS;
static boolean pay_billed_items(struct monst *, int, Bill *, boolean, boolean *) NONNULLPTRS;
static void update_bill(int, int, Bill *, struct eshk *, struct bill_x *, struct obj *) NONNULLPTRS;
static int dopayobj(struct monst *, struct bill_x *, struct obj *, int, boolean, boolean) NONNULLPTRS;
static int buy_container(struct monst *, int, int, Bill *) NONNULLPTRS;
static void reject_purchase(struct monst *, struct obj *, long) NONNULLPTRS;
static boolean insufficient_funds(struct monst *, struct obj *, long) NONNULLPTRS;
...
money2u(struct monst *mon, long amount)
{
    ...
    if (!merge_choice(invent, mongold) && inv_cnt(FALSE) >= invlet_basic) {
    ...
    struct obj *otmp)
{
    ...
    if (otmp->no_charge) {
        ...
        if (!shkp ||
             ...
             (rm_shkp = rooms[rno - ROOMOFFSET].resident) == 0 ||
              rm_shkp == shkp) {
            otmp->no_charge = 0;
        }
    }
...
setpaid(struct monst *shkp)
{
    ...
    clear_no_charge(shkp, level.buriedobjlist);
...
static void
deserted_shop(char *enterstring)
/*const*/
{
    struct monst *mtmp;
    ...
    for (x = r->lx; x <= r->hx; ++x) {
        for (y = r->ly; y <= r->hy; ++y) {
            ...
            if ((mtmp = m_at(x, y)) != 0) {
                ++n;
                if (sensemon(mtmp) ||
                     ((M_AP_TYPE(mtmp) == M_AP_NOTHING || M_AP_TYPE(mtmp) == M_AP_MONSTER) && canseemon(mtmp))) {
                    ++m;
                }
            }
        }
...
u_entered_shop(char *enterstring)
{
    ...
    if (!shkp) {
        if (!strchr(empty_shops, *enterstring) &&
             in_rooms(u.ux, u.uy, SHOPBASE) != in_rooms(u.ux0, u.uy0, SHOPBASE)) {
        ...
        if (pick || mattock || crystal_pick) {
            ...
            if (crystal_pick) {
                ...
                while ((crystal_pick = crystal_pick->nobj) != 0) {
                    if (crystal_pick->otyp == CRYSTAL_PICK) {
                        ++cnt;
                    }
                }
            }
...
pick_pick(struct obj *obj)
{
    ...
    if (shkp && inhishop(shkp)) {
        ...
        if (moves != pickmovetime) {
            if (!Deaf && !muteshk(shkp)) {
                verbalize("You sneaky %s!  Get out of here with that pick!", cad(FALSE));
            } else {
                pline("%s %s your pick!",
                      Shknam(shkp),
...
onbill(struct obj *obj, struct monst *shkp, boolean silent)
{
    if (shkp) {
        ...
        for (ct = ESHK(shkp)->billct, bp = ESHK(shkp)->bill_p;
            ...
            if (bp->bo_id == obj->o_id) {
                if (!obj->unpaid) {
                    warning("onbill: paid obj on bill?");
                }
                return bp;
            }
        }
    ...
    if (obj->unpaid && !silent) {
        warning("onbill: unpaid obj %s?", !shkp ? "without shopkeeper" : "not on shk's bill");
    }
...
obfree(struct obj *obj, struct obj *merge)
{
    ...
    if ((bp = onbill(obj, shkp, FALSE)) != 0) {
        ...
        if (!bpm) {
            ...
            warning("obfree: not on bill, %s = (%d,%d,%ld,%d) (%d,%d,%ld,%d)?",
        ...
        if (merge &&
             (oid_price_adjustment(obj, obj->o_id) > oid_price_adjustment(merge, merge->o_id))) {
            merge->o_id = obj->o_id;
        }
    }
...
/* for itemized purchasing, create an alternate shop bill that hides
   container contents */
static int /* returns number of entries */
make_itemized_bill(
    struct monst *shkp,
    ...
    Bill *ibill) /* all used up items, if any, precede all intact items */
{
    ...
    for (i = 0; i < ibillct; ++i) {
        ...
        add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, 0, 0, ATR_NONE, buf,
                 MENU_ITEMFLAGS_NONE);
    }
    ...
    if (pay_done && !ANGRY(shkp) && paid) {
        if (!Deaf && !muteshk(shkp)) {
            ...
            pline("%s nods%s at you for shopping in %s %s%s",
                  Shknam(shkp),
                  !eshkp->surcharge ? " appreciatively" : "",
                  noit_mhis(shkp),
                  shtypes[eshkp->shoptype - SHOPBASE].name,
                  !eshkp->surcharge ? "!" : ".");
        }
    ...
    boolean *paid_p) /* output */
{
    ...
    do {
        if (via_menu /*&& more_than_one*/ ) {
            ...
            iprompt = !more_than_one ? 'y' : yn_function("Itemized billing?", "ynq m", 'q');
            if (iprompt == 'q') {
                return TRUE;
            }
            itemize = (iprompt == 'y');
...
{
    struct monst *mtmp, *mtmp2, *firstshk, *resident, *creditor, *hostile, *localshk;
    struct eshk *eshkp;
    ...
    boolean silently)
{
    ...
    if (numsk > 1) {
        if (cansee(shkp->mx, shkp->my) && croaked && !silently) {
            ...
            if (has_head(shkp->data) && rn2(2)) {
                Sprintf(takes, ", shakes %s %s,", noit_mhis(shkp), mbodypart(shkp, HEAD));
            }
    ...
    if (uinshop && inhishop(shkp) &&
       ...
       NOTANGRY(shkp) && !eshkp->following && u.ugrave_arise < LOW_PM) {
        taken = (invent != 0);
        if (taken && !silently) {
...
set_repo_loc(struct monst *shkp)
{
    ...
    /* savebones() sets u.ux,u.uy to 0,0 to remove hero from map but that
       ...
       play, and prior to adding 'if (repo.shopkeeper) return' above) */
    ox = u.ux ? u.ux : u.ux0;
...
finish_paybill(void)
{
    ...
    if (!isok(ox, oy)) {
        ...
        if (shkp) {
            warning("finish_paybill: bad location <%d,%d>.", ox, oy);
        }
    ...
    *nochrg = -1; /* assume 'not applicable' */
    if (*u.ushops &&
         ...
         *in_rooms(x, y, SHOPBASE) == *u.ushops &&
         (shkp = shop_keeper(inside_shop(x, y))) != 0 &&
         inhishop(shkp)) {
    ...
    struct monst *shkp) /* if angry, impose a surcharge */
{
    ...
    if (!obj->dknown || !objects[obj->otyp].oc_name_known) {
        if (obj->oclass == GEM_CLASS &&
            ...
            boolean pseudorand = ((game_seed % obj->otyp) >= obj->otyp / 2);
        ...
        if (!quietly) {
            if (is_izchak(shkp, TRUE) && !u.uevent.invoked) {
                if (Deaf || muteshk(shkp)) {
                    ...
                    if (obj->spe < 7) {
                        verbalize("You'll need %d%s candle%s to go along with it.",
                                (7 - obj->spe),
                                (obj->spe > 0) ? " more" : "",
                                plur(7 - obj->spe));
                    }
...
void
alter_cost(struct obj *obj, long amt)
    ...
    struct monst *shkp)
{
    ...
    if (dummy) {              /* a dummy object must be inserted into  */
        bp->useup = TRUE;     /* the billobjs chain here.  crucial for */
...
void
bill_box_content(struct obj *obj, boolean ininv, boolean dummy, struct monst *shkp)
{
    struct obj *otmp;
    ...
    if (!Deaf && !muteshk(shkp) && !silent) {
        ...
        if (!ininv) {
            pline("%s will cost you %ld %s%s.",
                  The(xname(obj)), ltmp, currency(ltmp), (obj->quan > 1L) ? " each" : "");
        } else {
            ...
            pline("%s %ld %s %s %s%s.\"",
                  buf, ltmp, currency(ltmp),
                  (save_quan > 1L) ? "per" : (contentscount && !obj->unpaid) ? "for the contents of this" : "for this",
                  xname(obj),
...
long
stolen_value(struct obj *obj, coordxy x, coordxy y, boolean peaceful, boolean silent)
{
    ...
    if (obj->oclass == COIN_CLASS) {
        ...
        if (Has_contents(obj)) {
            boolean ininv = (obj->where == OBJ_INVENT || obj->where == OBJ_FREE);
    ...
    if (peaceful) {
        ...
        if (!silent) {
            ...
            Sprintf(buf, "%sowe %s %ld %s", still, shkname(shkp), value, currency(value));
            if (u_count > 0) {
                ...
                Sprintf(eos(buf), " for %s", (obj->quan > 1L) ? "them" : "it");
            }
        ...
        if (!silent) {
            if (canseemon(shkp)) {
                Norep("%s booms: \"%s, you are a thief!\"", Shknam(shkp), plname);
            } else if (!Deaf) {
...
void
sellobj(struct obj *obj, coordxy x, coordxy y)
{
    struct monst *shkp;
    struct eshk *eshkp;
    ...
    if (!(isgold || cgold) &&
        ...
        if (!unpaid && (sell_how != SELL_DONTSELL) && !special_stock(obj, shkp, FALSE)) {
    ...
    if (!shkmoney) {
        ...
        if (!sell_response) {
            ...
            Sprintf(qbuf,
                    ...
                    Shknam(shkp), short_funds ? " only" : "",
                    offer, plur(offer),
                    (cltmp && !ltmp) ? ((yourc == 1L) ? "your item in " : "your items in ") : "",
                    obj->unpaid ? "the" : "your");
            one = !ltmp ? (yourc == 1L) : (obj->quan == 1L && !cltmp);
            Sprintf(qsfx, "%s.  Sell %s?",
                    (cltmp && ltmp) ? (only_partially_your_contents ?
                        ((yourc == 1L) ? " and item inside" : " and items inside") : and_its_contents) : "",
                    one ? "it" : "them");
            (void) safe_qbuf(qbuf, qbuf, qsfx, obj, xname, simpleonames, one ? "that" : "those");
        } else {
...
struct monst *
shkcatch(struct obj *obj, coordxy x, coordxy y)
{
    struct monst *shkp;
...
void
add_damage(coordxy x, coordxy y, long cost)
{
...
repairable_damage(struct damage *dam, struct monst *shkp)
{
    ...
    /* too soon to fix it? */
    if ((moves - dam->when) < REPAIR_DELAY) {
        return FALSE;
    }
...
{
    struct damage *dam = level.damagelist;
...
discard_damage_struct(struct damage *dam)
{
    ...
    if (dam == level.damagelist) {
        level.damagelist = dam->next;
    } else {
        struct damage *prev = level.damagelist;
...
{
    struct damage *dam = level.damagelist, *dam2, *prevdam = NULL;
    ...
    boolean catchup)
{
    ...
    if (ttmp) {
        ...
        case BEAR_TRAP:
            ...
            otmp = mksobj((ttmp->ttyp == LANDMINE) ? LAND_MINE : BEARTRAP, TRUE, FALSE);
            otmp->quan = 1L;
...
fix_shop_damage(void)
{
    ...
    for (shkp = next_shkp(fmon, FALSE); shkp;
        ...
        for (damg = level.damagelist; damg; damg = nextdamg) {
            nextdamg = damg->next;
...
pay_for_damage(const char *dmgstr, boolean cant_mollify)
{
    ...
    for (tmp_dam = level.damagelist; tmp_dam; tmp_dam = tmp_dam->next) {
        ...
        for (shp = shops_affected; *shp; shp++) {
            ...
            if (tmp_shk == shkp) {
                unsigned int damage_distance = distu(tmp_dam->place.x, tmp_dam->place.y);
...
shk_embellish(struct obj *itm, long cost)
{
    if (!rn2(3)) {
        ...
        case 4:
            ...
            if (objects[itm->otyp].oc_name_known ? objects[itm->otyp].oc_magic :
...
boolean
shk_has_leather_jackets(struct monst *shkp)
{
