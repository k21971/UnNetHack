...
#ifdef EXTRAINFO_FN
#include <sys/stat.h>
static char *dump_format_str(char *);
#endif
...
#ifdef EXTRAINFO_FN
/* NOTE: dump_format_str is not used for dumplog generation in
 * UnNetHack (see dump.c for that), it is used exclusively for
 * dgl-extrainfo
 */
static char *
...
done(int how)
{
    ...
    if (Lifesaved && (how <= MAX_SURVIVABLE_DEATH)) {
        ...
        if (uamul && uamul->cursed && rnf(1, 4)) {
            ...
            if (u.uhpmax <= 0) {
                u.uhpmax = (heaven_or_hell_mode ? 1 : 10); /* arbitrary */
            }
            savelife(how);
